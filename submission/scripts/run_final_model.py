...
from utils import read_yaml, load_train_test_nosplit_data, plot_coefficients, plot_predictions_with_ci
from tqdm import tqdm
# -----------------------------------------------------------------------------
# CLI
# -----------------------------------------------------------------------------
parser = argparse.ArgumentParser(description="Run GP regression ")
parser.add_argument("--seed", type=int, default=0, help="Random seed")
args = parser.parse_args()
seed = args.seed
np.random.seed(seed)
os.makedirs(f"results/GPRFinal_{seed}", exist_ok=True)
...
(
    ...
    *_,
    df_train_meta_train,
    scaler
) = load_train_test_nosplit_data(seed, variables)
...
kernel_roe_deer = RBF(active_dims=[104])
kernel_red_deer = RBF(active_dims=[105])
kernel_chinese_water_deer = RBF(active_dims=[106])
kernel_wood_mice_deer = RBF(active_dims=[107])
kernel_bank_voles_deer = RBF(active_dims=[108])
kernel_blackbird_deer = RBF(active_dims=[109])
...
# Fit GP model
# -----------------------------------------------------------------------------
model = GPR(data=(X_tensor, Y_tensor), kernel=kernel, mean_function=Constant(), noise_variance=1e-1)
opt = gpflow.optimizers.Scipy()
opt.minimize(model.training_loss, model.trainable_variables, options=dict(maxiter=100), method="L-BFGS-B")


log_dir = f"results/GPRFinal_{seed}/checkpoints"
save_dir = f"results/GPRFinal_{seed}/gpflow_model"
...
plot_coefficients(coef_wide, "Kernel Scales", os.path.join("results", f"GPRFinal_{seed}", "gp_kernel_scales.pdf"))
...
results_gp_file = os.path.join("results", f"GPRFinal_{seed}", "gpflow_results.json")
...
test_summary.to_csv(os.path.join("results", f"GPRFinal_{seed}", "test_predictions.csv"), index=False)
